...
目前的擴張／阻力主指數、驅動因素卡片及區域專案假設仍屬編輯式 MVP 輸入，不會只因 Action 執行而自動改變。等一級來源涵蓋足夠後，後續方法論版本才能以正規化觀察值計算這些內容。
...
事件發布後 30 天內顯示於「目前紀錄」，之後顯示於「封存」。兩者是同一批持久紀錄的不同檢視方式。成功更新會在靜態快照與 SQLite 中保留較舊事件及修訂版本；來源失敗則保留上次成功的事件並標示降級。目前提交的快照尚無通過驗證的基礎設施事件，因此首次成功更新找到合格紀錄前會顯示空狀態。
