...
const trendLabel = (trend, zh) => ({
  above: zh ? '短期價格趨勢偏正向' : 'Short-term price trend positive',
  below: zh ? '短期價格趨勢偏負向' : 'Short-term price trend negative',
  mixed: zh ? '價格趨勢混合' : 'Mixed price trend',
  unavailable: zh ? '趨勢資料不足' : 'Trend unavailable',
})[trend || 'unavailable'];
...
export default function App({ snapshot = {}, language = 'en', onLanguageChange = () => {} }) {
  ...
  return <main className="shell" lang={language}>
    <header>
      ...
      <div className="head-actions"><button className="language" onClick={() => onLanguageChange(zh ? 'en' : 'zh-TW')}>{zh ? 'EN' : '繁中'}</button><span className={`live ${route.view === 'Events' ? eventHealth?.status || 'pending' : snapshotMeta.generatedAt ? 'live' : 'pending'}`}>{route.view === 'Events' ? eventStatus : snapshotMeta.generatedAt ? t(`Snapshot ${dateLabel(snapshotMeta.generatedAt)}`, `快照 ${dateLabel(snapshotMeta.generatedAt)}`) : t('Snapshot loading', '快照載入中')}</span><Account language={language} weight={100} onPreferences={prefs => { if (['en', 'zh-TW'].includes(prefs.language)) onLanguageChange(prefs.language); }} /></div>
    </header>
